...
export default function PostPage({
  ...
  const summaryText = post.aiSummary || post.summary;
            ...
            <aside className="mt-16 rounded-2xl border border-accent-400/20 bg-accent-400/[0.04] p-6">
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-accent-200 uppercase tracking-widest">
                  {post.aiSummary ? 'AI · 一句话' : '摘要 · 一句话'}
                </span>
                {!post.aiSummary && (
                  <span className="text-[10px] text-neutral-500">（AI 未启用，作者手填）</span>
                )}
              </div>
              ...
              {post.aiQuestions && post.aiQuestions.length > 0 && (
                <>
                  <p className="mt-6 text-xs font-mono text-neutral-500 uppercase tracking-widest">
                    值得继续想
                  </p>
